...
export default function HomePage() {
        ...
        const loadDashboard = async () => {
            try {
                ...
                const requests = [
                    ...
                    api.get("/users", { params: { size: 100, sort: "dateRegister,desc" } }),
                    canReadAllLoans
    ...
    }, [users, currentMonth, previousMonth]);
    ...
    const monthlyLoans = useMemo(
        () => loans.filter((loan) => monthKey(loan.requestDate) === currentMonth).length,
        [loans, currentMonth]
    );
        ...
        <div className="page-content dashboard-page">
            ...
            <div className="stat-grid">
                ...
                <StatCard
                    ...
                    trend={loading ? null : userTrend}
                />
                <StatCard
                    label="Empréstimos no mês"
                    value={loading ? "—" : monthlyLoans}
                    description="Solicitações neste mês"
                    trend={loading ? null : loanTrend}
                />
            </div>
            ...
            <div className="dashboard-grid">
                <section className="dashboard-panel">
                    <div className="panel-heading">
                        <div><span className="eyebrow">ACERVO</span><h2>Destaques disponíveis</h2></div>
                ...
                <section className="dashboard-panel">
                    <div className="panel-heading">
                        <div><span className="eyebrow">CIRCULAÇÃO</span><h2>{canReadAllLoans ? "Mais emprestados" : "Meus empréstimos"}</h2></div>
                    ...
                    {canReadAllLoans ? (
                        ...
                        <div className="loan-summary-list">
                            {activeLoans.slice(0, 5).map((loan) => (
                                <div className="loan-summary" key={loan.id}><strong>{loan.bookTitle}</strong><span>{loan.status}</span></div>
                            ))}
