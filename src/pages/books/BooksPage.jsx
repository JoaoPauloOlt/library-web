...
export default function BooksPage() {
    ...
    const loadBooks = useCallback(async () => {
        try {
            ...
            if (search.trim()) {
                params[searchType === "author" ? "authorName" : searchType] = search.trim();
            }
        ...
        <div className="page-content">
            <div className="page-header page-header-inline">
                <div>
                    ...
                    <h1>Livros</h1>
                    <p>Explore o acervo e encontre livros disponíveis.</p>
                </div>

                {canCreateBook && (
                    <Link className="btn-primary" to="/books/new">+ Novo Livro</Link>
                )}
            </div>
            ...
            <div className="catalog-toolbar">
                <span>{booksPage.totalElements ?? 0} livro(s) encontrado(s)</span>
                {loading && <span>Carregando...</span>}
            </div>

            {!loading && !error && books.length === 0 && (
                <div className="empty-state card">Nenhum livro encontrado.</div>
            )}
                ...
                {books.map((book) => (
                    <article className="book-card" key={book.id}>
                        ...
                        <div className="book-info">
                            ...
                            <small>
                                {book.authors?.map((author) => author.name).join(", ") || "Autor não informado"}
                            </small>
                            <small>ISBN: {book.isbn}</small>

                            <div className="book-meta-row">
                                <span>{book.totalCopies ?? 0} exemplare(s)</span>
